...
#define LOG_FILE_MSG "[kafka AuthenConsumer.cpp]: "
class AuthenConsumerEventCb : public RdKafka::EventCb // RdKafka::EventCb  事件是从RdKafka传递错误、统计信息、日志等消息到应用程序的通用接口
{
public:
...
*/
class AuthenConsumerRebalanceCb : public RdKafka::RebalanceCb
{
public:
...
AuthenConsumer::AuthenConsumer(const std::string &brokers, const std::string &groupID,
                             const std::vector<std::string> &topics, int partition)
{
  ...
  m_event_cb = new AuthenConsumerEventCb;
  ...
  m_rebalance_cb = new AuthenConsumerRebalanceCb;
  ...
  // errorCode = m_config->set("sasl.username", "admin", errorStr);
	if (errorCode != RdKafka::Conf::CONF_OK)
	{
    cout << "Conf set failed:" << errorStr << std::endl;
		loggers->error("{}Conf set failed:{}", LOG_FILE_MSG, errorStr);
  ...
  // errorCode = m_config->set("sasl.password", "admin-secret", errorStr);
  if (errorCode != RdKafka::Conf::CONF_OK)
	{
    cout << "Conf set failed:" << errorStr << std::endl;
		loggers->error("{}Conf set failed:{}", LOG_FILE_MSG, errorStr);
  ...
  {
    loggers->error("{}Create AuthenConsumer failed: {}", LOG_FILE_MSG, errorStr);
  }
}

void AuthenConsumer::pullMessage()
{
  auto loggers = spdlog::get("project_log");
  // 订阅Topic
  ...
  while (true)
  {
    ...
    consume_msg(msg, NULL);
    delete msg;
  }
}

AuthenConsumer::~AuthenConsumer()
{
  m_consumer->close();
  delete m_config;
...
void deal_info_consume(RdKafka::Message *msg)
{
  ...
  MyTimer obj;
	obj.start(3000, [package]() { printNum(package); });
  obj.stop();
}
