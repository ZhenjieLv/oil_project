...
#define LOG_FILE_MSG "[kafka KafkaProducer.cpp]: "
// call back
...
public:
	void dr_cb(RdKafka::Message &message) // 重载基类RdKafka::DeliveryReportCb中的虚函数dr_cb()
	{
		...
		if (message.err() != 0) // 发送出错
		{
			...
			loggers->error("{}Message delivery failed: {}", LOG_FILE_MSG, message.errstr());
			sprintf(tmp, "网络错误,无法发送到kafka:%s", message.errstr().c_str());
...
// if(!insert_status_alarm_info(2,tmp))
  //loggers->error("{}插入报警表失败！", LOG_FILE_MSG);// 插入报警表 报警原因是“网络错误，无法发送到kafka，message.errstr()”
		...
		{
			cout << "Message delivered success!" << endl;
		}
#endif
	}
};
...
KafkaProducer::~KafkaProducer()
{
	...
	{
		loggers->info("{}producer等待所有未完成的produce请求数量：{}", LOG_FILE_MSG, m_producer->outq_len());
...
bool KafkaProducer::pushMessage(const std::string &str)
{
	auto loggers = spdlog::get("project_log");
...
//    //printf("before_producer\n");
//    while(true)
//    {
//     //printf("producer!\n");
//     // char msg[64] = {0};
//     // sprintf(msg, "%s", "Hello RdKafka ");
//     // printf("msg\n");
//      // 生产消息
//      uint8_t msg[5] = { 0xAA, 0x01, 0xE3, 0x23, 0x34 };
//      uint32_t len = 5;
//      char key[8] = {0};      // 主要用来做负载均衡
//      sprintf(key, "%d", 2);
//      producer.pushMessage(msg, key, len);
//     //producer.pushMessage(msg, nullptr);
...
// {
//   KafkaProducer producer("10.16.14.20:9092", "test");
//  if(producer.pushMessage(str))
//     return true;
//   else
//     return false;
// }
